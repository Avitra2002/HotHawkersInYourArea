...
const PreferencesPage = () => {
  ...
  // Set default values for selected canteen and store
  useEffect(() => {
    if (selectedCanteen === 'SUTD Canteen' && !selectedStore) {
      setSelectedStore('Taiwanese'); // Default store for SUTD Canteen
    }
  }, [selectedCanteen, selectedStore, setSelectedCanteen, setSelectedStore]);

  const handleCanteenChange = (event) => {
    const selected = event.target.value;
    setSelectedCanteen(selected);

    // Set the first store as default for the selected canteen
    const defaultStore = canteenStores[selected]?.[0] || null;
